...
- `federated_meds_v.drug_supply_days` is a VARCHAR in the prod warehouse and carries `''` (empty string, not NULL) for missing values. `MedicationItem.drug_supply_days`/`number_of_refills` use `BlankableInt` (blank string → None) — don't retighten them to plain `Optional[int]`: on 2026-07-06 that validation error killed every meds retrieval for ADT-feed patients and sent the agent into tool-retry loops, while the SQL itself succeeded (the run log's `result_summary` showed a misleading `empty_result`). The synthetic fixture (`tests/agent/redshift_synthetic.sql`) deliberately seeds a `''` value to pin this.
- `federated_allergies_v` has NO `onset_date` or `status_datetime` column (the view was reshaped — they errored on every allergies question). The clinical event date is `date` (frequently NULL in prod, ~79% of rows) and `created_date` is the record timestamp. `agent/db/queries/allergies.py` coalesces `COALESCE("date", created_date)` for ordering/date-filtering (so NULL clinical dates fall back rather than being silently dropped) and aliases `"date" AS onset_date` to keep the tool's `AllergyItem.onset_date` field stable. Quote `"date"` — it's a reserved-ish identifier. The column list is pinned by `tests/sample_db/test_schema_matches_redshift.py` against the Redshift catalog JSON; keep `scripts/sample_db/schema.sql` in sync. The real prod schema (allergy/reaction/type/severity/status/comments/code/code_type plus location/encounter cols) is the source of truth — confirmed 2026-06-26.
