...
class TestVannaServiceExceptions:
    ...
    def test_init_exception_anthropic(self):
        ...
        with patch("utils.vanna_calls.VannaDB_VectorStore.__init__", side_effect=Exception("VannaDB initialization error")):
    ...
    def test_init_exception_anthropic_chromadb(self):
        ...
        with patch("utils.vanna_calls.ChromaDB_VectorStore.__init__", side_effect=Exception("ChromaDB initialization error")):
            # Should catch the exception and log it
...
class TestTrainingFunctionExceptions:
    ...
    def test_train_file_exception_train(self, mock_st_error):
        ...
        with patch("utils.vanna_calls.VannaService.get_instance") as mock_get_instance, \
             patch("builtins.open", mock_open(read_data='{"sample_queries": [{"question": "q", "query": "sql"}], "sample_documents": []}')), \
