...
def test_summary_uses_status_and_write_stream(monkeypatch):
    ...
    fake_state = _State()
    fake_state.update(
        dict(messages=[], show_sql=True, show_table=False, show_chart=False, show_summary=True, speak_summary=False)
    )
    ...
    monkeypatch.setattr(
        cbh, "get_summary_event_stream", lambda q, df, think=False: iter([("content", "hello "), ("content", "world")])
    )
