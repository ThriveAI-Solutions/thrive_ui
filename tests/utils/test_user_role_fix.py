...
class TestUserRoleFix:
    ...
    def test_get_vanna_service_uses_admin_role(
        self, mock_streamlit_session_admin, mock_streamlit_secrets, mock_user_preferences
    ):
    ...
    def test_get_vn_function_preserves_admin_role(
        self, mock_streamlit_session_admin, mock_streamlit_secrets, mock_user_preferences
    ):
    ...
    def test_user_role_not_defaulting_to_patient_during_question(
        self, mock_streamlit_session_admin, mock_streamlit_secrets, mock_user_preferences
    ):
    ...
    def test_training_data_filtering_uses_correct_role(
        self, mock_streamlit_session_admin, mock_streamlit_secrets, mock_user_preferences
    ):
        ...
        with patch.object(VannaService, "_setup_vanna"):
            ...
            mock_vn_instance._prepare_retrieval_metadata.return_value = {
                "user_role": {"$gte": RoleTypeEnum.ADMIN.value}
            }
...
class TestUserRoleEdgeCases:
    ...
    def test_user_preferences_called_before_service_creation(
        self, mock_streamlit_session_admin, mock_streamlit_secrets, mock_user_preferences
    ):
