...
def mock_vanna_service_with_in_memory_chroma(thrive_ai_chromadb_in_memory):
    ...
    with patch("streamlit.session_state") as mock_st_session_state:
        mock_st_session_state.get.return_value = RoleTypeEnum.ADMIN.value # Match user_role in thrive_ai_chromadb_in_memory
...
def test_train_ddl_adds_to_chromadb_with_user_role(mock_read_forbidden, mock_psycopg2_connect, mock_streamlit_secrets_ddl, mock_vanna_service_with_in_memory_chroma):
    # 1. Setup Mocks
...
def test_train_ddl_describe_to_rag_adds_documentation_with_user_role(
    ...
    mock_cursor.description = [("id",), ("name",)] # (name, type_code, display_size, internal_size, precision, scale, null_ok)
    ...
    mock_submit_prompt.side_effect = [
...
def test_role_based_access_with_different_service_roles(thrive_ai_chromadb_in_memory):
    ...
    admin_user_service = type(thrive_ai_chromadb_in_memory)(user_role=0, client=thrive_ai_chromadb_in_memory.client)  # Admin = 0
    
    # Clear ALL existing documents from all collections
    for collection in [admin_user_service.ddl_collection, admin_user_service.documentation_collection, admin_user_service.sql_collection]:
...
def test_role_access_with_gte_filter(mock_vanna_service_with_in_memory_chroma):
    ...
    for role, doc_name in [(1, "Low privilege doc"), (2, "Medium privilege doc"), (3, "High privilege doc"), (4, "Super admin doc")]:
        service.vn.documentation_collection.add(
            documents=[doc_name],
...
def test_combined_metadata_filters_with_roles(mock_vanna_service_with_in_memory_chroma):
    ...
    results = service.vn.documentation_collection.get(where={
        "$and": [
            {"user_role": {"$lte": 2}},
            {"category": "api"}
        ]
...
def test_train_ddl_shows_success_messages(mock_warning, mock_success, mock_toast, mock_read_forbidden, mock_psycopg2_connect, mock_streamlit_secrets_ddl, mock_vanna_service_with_in_memory_chroma):
...
@patch("utils.vanna_calls.st.success")
@patch("utils.vanna_calls.st.warning")
def test_train_ddl_shows_warning_when_no_tables(mock_warning, mock_success, mock_toast, mock_read_forbidden, mock_psycopg2_connect, mock_streamlit_secrets_ddl, mock_vanna_service_with_in_memory_chroma):
