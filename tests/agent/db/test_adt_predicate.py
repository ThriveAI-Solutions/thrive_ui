...
def test_flag_sql_uses_bool_or_for_redshift():
    ...
    assert "NOT BOOL_OR(COALESCE(adt.clean_status, '') = 'CANCEL ADMIT')" in sql
...
def test_flag_sql_cancel_check_is_null_safe_for_redshift():
    sql = inpatient_admission_flag_sql("redshift", alias="adt")
    assert "adt.clean_status = 'CANCEL ADMIT'" not in sql
    assert "COALESCE(adt.clean_status, '') = 'CANCEL ADMIT'" in sql
...
def test_cohort_subquery_distinct_for_filter():
    ...
    n = " ".join(sql.split())
    assert "SELECT DISTINCT CAST(adt.patient_id AS VARCHAR) AS patient_id" in n
    assert "FROM dw.federated_adt_v adt" in n
    assert "GROUP BY CAST(adt.patient_id AS VARCHAR), COALESCE(NULLIF(TRIM(CAST(adt.visit_number AS VARCHAR)), '')" in n
...
def test_cohort_subquery_projects_admit_date_for_breakdown():
    ...
    n = " ".join(sql.split())
    assert "SELECT CAST(adt.patient_id AS VARCHAR) AS patient_id" in n and "DISTINCT" not in n
