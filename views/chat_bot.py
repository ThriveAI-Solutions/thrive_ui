            ...
            case MessageType.SUMMARY.value:
                ...
                with cols[2]:
                    with st.popover("Actions", use_container_width=True):
                        st.button(
                            "Speak Summary", key=f"speak_summary_{index}", on_click=lambda: speak(message.content)
                        )
                        ...
                        if st.button("Generate Table", key=f"table_{index}"):
                            add_message(
                                Message(
                                    RoleType.ASSISTANT, my_df, MessageType.DATAFRAME, message.query, message.question
                                )
                            )
                        st.button(
                            "Generate Graph",
