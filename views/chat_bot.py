...
st.sidebar.button("Reset", on_click=lambda: cbh.set_question(None), use_container_width=True, type="primary")
...
if st.session_state.get("show_suggested", True):
    with st.sidebar.popover("Click to show suggested questions", use_container_width=True):
        questions = cbh.vn.generate_questions()
...
if st.session_state.get("show_question_history", True):
    ...
    filtered_messages = cbh.get_unique_messages()
...
for message in st.session_state.messages:
    cbh.render_message(message, index)
    index = index + 1
...
if my_question:
    ...
    sql, elapsed_time = cbh.vn.generate_sql(question=my_question)
    st.session_state.my_question = None

    if sql:
        if cbh.vn.is_sql_valid(sql=sql):
        ...
        if st.session_state.get("show_table", True):
            ...
            cbh.add_message(Message(RoleType.ASSISTANT, df, MessageType.DATAFRAME, sql, my_question))

        if st.session_state.get("show_chart", True):
            cbh.get_chart(my_question, sql, df)
        ...
        if st.session_state.get("show_followup", True):
            cbh.get_followup_questions(my_question, sql, df)
    else:
        cbh.add_message(
            Message(
                RoleType.ASSISTANT,
