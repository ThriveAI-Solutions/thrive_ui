...
def render_agent_artifacts(response, question: str) -> None:
    """Translate AgentResponse.artifacts into chat Messages.

    Each artifact variant maps to a MessageType so the existing
    MESSAGE_RENDERERS dispatch handles it. Phase 3 design §3.3.

    The Chart and Summary variants ship in PR 4 and PR 5; for PR 3 we
    only handle SQL and DataFrame. The other branches are no-ops here
    and will be filled in by those PRs.
    """
    ...
    for artifact in response.artifacts:
        if isinstance(artifact, SqlArtifact):
            add_message(
                Message(
                    RoleType.ASSISTANT,
                    artifact.sql,
        ...
        elif isinstance(artifact, DataFrameArtifact):
            import pandas as pd
