    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            embed = {
                ...
                "fields": [
                    {"name": "User", "value": DiscordHandler.current_user, "inline": True},
            ...
            threading.Thread(target=self._send_to_discord, args=(payload,), daemon=True).start()
...
def add_discord_handler_if_configured(logger: logging.Logger) -> None:
    ...
    if webhook_url:
        ...
        formatter = logging.Formatter("{asctime} {levelname:<8} {name}: {message}", style="{", datefmt="%H:%M:%S")
