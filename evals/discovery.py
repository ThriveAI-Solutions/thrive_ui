...
def suggest_patients(adapter, date_start: str, date_end: str, per_domain: int = 5) -> dict:
    """adapter: AnalyticsDbAdapter (duck-typed: fetch_all + schema_prefix)."""
    # Timestamp columns compared against a bare date string would exclude
    # everything after midnight on date_end; use an exclusive next-day bound
    # so the window covers date_end in full.
    end_exclusive = (date.fromisoformat(date_end) + timedelta(days=1)).isoformat()
    out: dict[str, list[dict]] = {}
    for probe in DOMAIN_PROBES:
        sql = (
            f"SELECT source_id, COUNT(*) AS record_count "
            f"FROM {adapter.schema_prefix}{probe['view']} "
            f"WHERE {probe['date_column']} >= :date_start AND {probe['date_column']} < :date_end_exclusive "
            f"AND source_id IS NOT NULL "
            f"GROUP BY source_id ORDER BY record_count DESC LIMIT {int(per_domain)}"
        )
        try:
            rows = adapter.fetch_all(sql, {"date_start": date_start, "date_end_exclusive": end_exclusive})
